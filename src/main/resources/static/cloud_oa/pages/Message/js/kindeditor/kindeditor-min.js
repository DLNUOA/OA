...
(function(A, i) {
	...
	function za(a, b) {
		...
		function f(a, b) {
			...
			else if (j = /\[((?:[\w\-]|\\.)+)\]/.exec(a)) c =
				h(j[1].toLowerCase(), null, k, b);
	...
	function Ma(a, b, c) {
		...
		function g(e,
			...
			for (var i = e.firstChild, o; i;) {
				...
				if (n > 0)
					if (i.nodeType == 1)
						if (p >= 0 && s <= 0) b && l.appendChild(i.cloneNode(!0)), c && f.push(i);
						else {
							var M;
							b && (M = i.cloneNode(!1), l.appendChild(M));
							if (g(i, M) === !1) return !1
						} else
				if (i.nodeType == 3 && (i = i == k.startContainer ? d(i, k.startOffset, i.nodeValue.length) :
	...
	function Xb(a, b, c, d) {
		var e = [Va === "" ? "<html>" : '<html dir="' + Va + '">', '<head><meta charset="utf-8" /><title></title>', "<style>", "html {margin:0;padding:0;}", "body {margin:0;padding:5px;}", 'body, td {font:14px/1.5 "sans serif",tahoma,verdana,helvetica;}', "body, p, div {word-wrap: break-word;}", "p {margin:5px 0;}", "table {border-collapse:collapse;}", "img {border:0;}", "noscript {display:none;}", "table.ke-zeroborder td {border:1px dotted #AAA;}",
	...
	function Za(a) {
		for (var b =
			document.getElementsByTagName("head")[0] || (N ? document.body : document.documentElement), c = document.createElement("link"), d = Db(ha(a, "absolute")), e = f('link[rel="stylesheet"]', b), g = 0, h = e.length; g < h; g++)
			if (Db(ha(e[g].href, "absolute")) === d) return;
		b.appendChild(c);
		c.href = a;
		c.rel = "stylesheet"
	}
		...
		E(D, {
			...
			pos: function() {
				var a = this[0],
					...
					c = 0;
				if (a)
					if (a.getBoundingClientRect) a = a.getBoundingClientRect(), c = aa(this.doc), b = a.left + c.x, c = a.top + c.y;
					else
		...
		E(Xa, {
			init: function(a) {
				var b = f(a.button),
					...
					d = a.url || "",
					e = b.val(),
					g = a.extraParams || {}, h = b[0].className || "",
...
//				}) : i.last().css("visibility", "hidden"));
			...
			exec: function(a) {
				var a = a.toLowerCase(),
					b = this.cmd,
					c = L(a, "selectall,copy,paste,print".split(",")) <
						0;
